...
### 案例分析 left join 后 and 和 where 的比较
```
(root@localhost)[test]> create table a (id int ,sid int ,utype varchar(10));
Query OK, 0 rows affected (0.14 sec)

(root@localhost)[test]> insert into a values (1,1,'a'),(2,1,'b'),(3,2,'c'),(4,3,'d');
Query OK, 4 rows affected (0.11 sec)
Records: 4  Duplicates: 0  Warnings: 0

(root@localhost)[test]> create table b (sid int,remark varchar(10));
Query OK, 0 rows affected (0.02 sec)

(root@localhost)[test]> insert into b values (1,'a'),(2,'B'),(3,'C'),(4,'D');
...
// 外链接 outer join 分三部：产生笛卡尔积、应用ON过滤器、添加外部行.
// 对于保留表中的行数据，如果是未找到匹配数据而被添加的记录，其值用NULL填充。
(root@localhost)[test]> select * from a left join b on a.sid=b.sid and b.sid = 1;
+------+------+-------+------+--------+
...
(root@localhost)[test]> select * from a left join b on a.sid=b.sid where b.sid=1;
+------+------+-------+------+--------+
| id   | sid  | utype | sid  | remark |
+------+------+-------+------+--------+
|    1 |    1 | a     |    1 | a      |
...
// 内连接没有这个 and where的区别。都可以。
(root@localhost)[test]> select * from a inner join b on a.sid=b.sid where b.sid=1;
+------+------+-------+------+--------+
| id   | sid  | utype | sid  | remark |
+------+------+-------+------+--------+
|    1 |    1 | a     |    1 | a      |
...
(root@localhost)[test]> select * from a inner join b on a.sid=b.sid and b.sid=1;
+------+------+-------+------+--------+
| id   | sid  | utype | sid  | remark |
+------+------+-------+------+--------+
|    1 |    1 | a     |    1 | a      |
...
// 目前两个表都没有索引，连接的时候使用了 Block Nested Loop 算法。
(root@localhost)[test]> explain select * from a left join b on a.sid=b.sid where  b.sid=1;
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+----------------------------------------------------+
| id | select_type | table | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra                                              |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+----------------------------------------------------+
|  1 | SIMPLE      | a     | NULL       | ALL  | NULL          | NULL | NULL    | NULL |    4 |    25.00 | Using where                                        |
|  1 | SIMPLE      | b     | NULL       | ALL  | NULL          | NULL | NULL    | NULL |    4 |    25.00 | Using where; Using join buffer (Block Nested Loop) |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+----------------------------------------------------+
...
##### 针对上面的案例，如何最大限度优化这条sql？select utype from a where sid =1
```
(root@localhost)[test]> explain select utype from a where sid =1;
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
| id | select_type | table | partitions | type | possible_keys | key  | key_len | ref  | rows | filtered | Extra       |
+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+-------------+
|  1 | SIMPLE      | a     | NULL       | ALL  | NULL          | NULL | NULL    | NULL |    4 |    25.00 | Using where |
...
// 创建二级索引 需要回表
(root@localhost)[test]> alter table a add index idx_sid(sid);
...
| id | select_type | table | partitions | type | possible_keys | key     | key_len | ref   | rows | filtered | Extra |
+----+-------------+-------+------------+------+---------------+---------+---------+-------+------+----------+-------+
|  1 | SIMPLE      | a     | NULL       | ref  | idx_sid       | idx_sid | 5       | const |    2 |   100.00 | NULL  |
+----+-------------+-------+------------+------+---------------+---------+---------+-------+------+----------+-------+
1 row in set, 1 warning (0.00 sec)

(root@localhost)[test]> alter table a drop index idx_sid;
Query OK, 0 rows affected (0.03 sec)
Records: 0  Duplicates: 0  Warnings: 0

(root@localhost)[test]> show create table a;
+-------+-------------------------------------------------------------------------------------------------------------------------------------------------------+
| Table | Create Table                                                                                                                                          |
+-------+-------------------------------------------------------------------------------------------------------------------------------------------------------+
| a     | CREATE TABLE `a` (
...
// 创建联合索引
(root@localhost)[test]> alter table a add index idx_sid(sid,utype);
...
| id | select_type | table | partitions | type | possible_keys | key     | key_len | ref   | rows | filtered | Extra       |
+----+-------------+-------+------------+------+---------------+---------+---------+-------+------+----------+-------------+
|  1 | SIMPLE      | a     | NULL       | ref  | idx_sid       | idx_sid | 5       | const |    2 |   100.00 | Using index |
+----+-------------+-------+------------+------+---------------+---------+---------+-------+------+----------+-------------+
1 row in set, 1 warning (0.00 sec)
...
1. 获取各朝代（dept）分数最高的男性（sex=1）的用户信息
2. 统计朝代总分大于200的朝代

SELECT *
FROM c, (SELECT dept, MAX(score) AS maxscore
	FROM c
	WHERE sex = 1
	GROUP BY dept
	) cg
WHERE c.dept = cg.dept
	AND cg.maxscore = c.score
	

SELECT *
FROM c RIGHT JOIN (SELECT dept, MAX(score) AS maxscore
	FROM c
	WHERE sex = 1
	GROUP BY dept
	) cg ON c.dept = cg.dept
AND cg.maxscore = c.score

+----+-----------+------+-------+--------+--------+----------+
| id | name      | sex  | score | dept   | dept   | maxscore |
+----+-----------+------+-------+--------+--------+----------+
|  1 | 嬴政      |    1 |    96 | 秦汉   | 秦汉   |       96 |
|  6 | 曹操      |    1 |    62 | 三国   | 三国   |       62 |
| 10 | 李世民    |    1 |    82 | 唐朝   | 唐朝   |       82 |
+----+-----------+------+-------+--------+--------+----------+

SELECT dept
	, substring_index(GROUP_CONCAT(name ORDER BY score DESC, id ASC), ',', 1)
	, MAX(score)
FROM c
WHERE sex = 1
GROUP BY dept;
+--------+-------------------------------------------------------------------------+------------+
| dept   | substring_index(GROUP_CONCAT(name ORDER BY score DESC, id ASC), ',', 1) | MAX(score) |
+--------+-------------------------------------------------------------------------+------------+
| 三国   | 曹操                                                                    |         62 |
| 唐朝   | 李世民                                                                  |         82 |
| 秦汉   | 嬴政                                                                    |         96 |
...
WHERE id IN (
	...
	FROM c
		RIGHT JOIN (
			SELECT dept, MAX(score) AS maxscore
			FROM c
			WHERE sex = 1
			GROUP BY dept
		) cg
...
SELECT dept, SUM(score) AS sumscore
FROM c
GROUP BY dept
HAVING sumscore > 200

(root@localhost)[test]> SELECT dept, SUM(score) AS sumscore
    -> FROM c
    -> GROUP BY dept
    -> HAVING sumscore > 200
    -> ;
...
SELECT DISTINCT c.dept, sumscore
FROM c
	LEFT JOIN (
		SELECT dept, SUM(score) AS sumscore
		FROM c
